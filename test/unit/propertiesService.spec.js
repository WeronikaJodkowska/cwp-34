...
describe('services/propertiesService', () => {
	...
	describe('readById', () => {
		...
		it('should call db.findByPk', async () => {
			await instance.readById(id);

			expect(repository.findByPk).toHaveBeenCalled();
			expect(repository.findByPk.mock.calls[0][0]).toEqual(1);
			expect(repository.findByPk.mock.calls[0][1]).toEqual({ raw: true });
		})
	...
	describe('update', () => {
		...
		it('should call db.update and db.readById', async () => {
			...
			expect(repository.findByPk).toHaveBeenCalled();
			expect(repository.findByPk.mock.calls[0][0]).toEqual(2);
		});
	...
	describe('bindAgent', () => {
		...
		it('should call db.findByPk', async () => {
			...
			expect(agentsRepository.findByPk).toHaveBeenCalled();
			expect(agentsRepository.findByPk.mock.calls[0][0]).toEqual(1);
		})
